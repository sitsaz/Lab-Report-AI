...
import { Beaker, ArrowLeft, Key, Languages, BookOpen, AlertTriangle, Save, FolderOpen, History, MessageSquare, ListTodo, Download } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    if (savedData) {
      try {
        ...
        if (parsed.file && parsed.lastSave) {
          // Silent check, we will offer a "Restore" button on landing
        }
      } catch (e) {
  ...
  const exportSession = () => {
    ...
    a.href = url;
    a.download = `LabReport_Session_${f.name.replace('.docx', '')}.json`;
    a.click();
  ...
  const handleBulkResolveConflicts = async (resolutions: Record<string, 'kept_existing' | 'updated_new' | 'combined'>) => {
    ...
    const resolutionDetails = Object.entries(resolutions).map(([id, res]) => {
      ...
      if (res === 'updated_new') {
        ...
        return `Conflict "${msg.conflict.description}": USER DECISION = COMBINE/MERGE. Existing: "${msg.conflict.existing_info}" New: "${msg.conflict.new_info}". Synthesize BOTH into professional rewrite.`;
      }
      return `Conflict "${msg.conflict.description}": USER DECISION = KEEP ORIGINAL.`;
    }).filter(Boolean).join("\n");

    const finalPrompt = `[SYSTEM: RESOLUTION MANIFEST]\n${resolutionDetails}\n\nCRITICAL: Execute all 'OVERWRITE/COMBINE' via 'update_report' and finish the task.`;
    setActiveTab('chat');
  ...
  const handleInsertToDoc = (textToInsert: string) => {
    if (!currentFile) return;
    const newContent = currentFile.content.substring(0, cursorPosition) + textToInsert + currentFile.content.substring(cursorPosition);
  ...
  if (!currentFile) {
    const hasAutosave = !!localStorage.getItem(STORAGE_KEY);
    return (
        ...
        <div className="absolute top-6 right-6 flex items-center gap-2">
          {hasAutosave && (
            <button 
              ...
              className="flex items-center gap-2 px-3 py-2 bg-amber-50 text-amber-700 rounded-lg border border-amber-200 shadow-sm hover:bg-amber-100 transition-all text-sm font-medium"
            >
          ...
          <button onClick={() => setLanguage(language === 'en' ? 'fa' : 'en')} className="px-3 py-2 bg-white text-slate-600 rounded-lg border border-slate-200 shadow-sm hover:text-indigo-600 transition-all text-sm font-medium uppercase">
            <Languages className="w-4 h-4 inline-block mr-1" />{language}
          </button>
        </div>
        <div className="w-full max-w-2xl text-center mb-10">
          <h1 className="text-4xl font-bold text-slate-900 mb-4 tracking-tight">Scientific Lab Report Assistant</h1>
          <p className="text-lg text-slate-500">Upload your report or a saved session file to continue.</p>
        </div>
        <FileUploader 
    ...
    <div className="h-screen flex flex-col bg-slate-100 overflow-hidden">
      <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between px-4 shadow-sm z-10">
        ...
        <div className="flex items-center gap-2">
          <div className="flex bg-slate-50 rounded-lg p-1 border border-slate-200 mr-2">
            <button 
              onClick={exportSession}
              className="flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-slate-600 hover:text-indigo-600 hover:bg-white rounded-md transition-all"
              title="Save Session to File"
            >
              <Save className="w-3.5 h-3.5" />Save
            </button>
            <div className="w-[1px] h-4 bg-slate-200 mx-1 self-center"></div>
            <label className="flex items-center gap-1.5 px-3 py-1 text-xs font-semibold text-slate-600 hover:text-indigo-600 hover:bg-white rounded-md transition-all cursor-pointer">
              <FolderOpen className="w-3.5 h-3.5" />Load
              <input type="file" accept=".json" className="hidden" onChange={(e) => e.target.files && handleImportSession(e.target.files[0])} />
            </label>
          </div>
          <button onClick={() => setLanguage(language === 'en' ? 'fa' : 'en')} className="p-1.5 text-slate-400 hover:text-indigo-600 uppercase text-xs font-bold border border-transparent hover:border-slate-200 rounded-lg px-2">
      ...
      <main className="flex-1 flex overflow-hidden p-4 gap-4">
        <div className="w-full md:w-[400px] lg:w-[450px] flex-shrink-0 flex flex-col h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="flex border-b border-slate-100 overflow-x-auto">
            <button onClick={() => setActiveTab('chat')} className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'chat' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/10' : 'border-transparent text-slate-500'}`}><MessageSquare className="w-4 h-4 inline mr-2" />Chat</button>
            <button onClick={() => setActiveTab(unresolvedConflicts.length > 0 ? 'conflicts' : 'tasks')} className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'tasks' || activeTab === 'conflicts' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/10' : 'border-transparent text-slate-500'}`}>
            ...
            <button onClick={() => setActiveTab('citations')} className={`flex-1 py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === 'citations' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/10' : 'border-transparent text-slate-500'}`}><BookOpen className="w-4 h-4 inline mr-2" />Citations</button>
          </div>
